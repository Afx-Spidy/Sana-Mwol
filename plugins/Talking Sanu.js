...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to sana voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
Sana.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
	var plkser = await addplk(message.jid);
    if (message.message.startsWith('sana') && !plkser.message.includes('{sana}')) {        
...
Sana.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
	var plkser = await addplk(message.jid);
        if (plkser.message.includes('{sana}') && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
Sana.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
Sana.addCommand({ pattern: 'sana ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.sana on / off' }, (async (message, match) => {
    var sana_status = `${conf.TALKING_SANA}`
    if (match[1] == 'on') {
        if (sana_status == 'true') {
